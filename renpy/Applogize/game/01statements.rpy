...
python early in phone:
    ...
    class _RawInitPhoneRegister(_RawPhoneRegister):
        ...
        def _execute(self):
            ...
            globals = store.__dict__
            if self.default_statement is not None:
                # Ren'Py 8+ may wrap in Init or use .variable instead of .varname
                ds = self.default_statement
                varname = getattr(ds, 'varname', None) or getattr(ds, 'variable', None) or getattr(ds, 'name', None)
                if varname is None and hasattr(ds, 'block') and ds.block:
                    first = ds.block[0] if hasattr(ds.block, '__getitem__') else getattr(ds.block, 'block', [None])[0]
                    if first is not None:
                        varname = getattr(first, 'varname', None) or getattr(first, 'variable', None) or getattr(first, 'name', None)
                if varname is not None:
                    gc = getattr(store, varname, None)
            else:
